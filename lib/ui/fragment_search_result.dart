...
class _SearchResultFragmentState extends State<SearchResultFragment> {
  ...
  StreamController<List<Customer>> _controller = StreamController<List<Customer>>();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_isSearching ? "Searching" : "${_list.length} ${_list.length > 1 ? "results" : "result"} found"),
      ),
  ...
  Future getData() async {
    ...
    var result =
    await http.post(BASE_URL + API_SEARCH, headers: headers);
